...
/**
 * 1.1.1 = 1*100*100 + 1*100 + 1 1.2.2 = 1*100*100 + 2*100 + 3
 * 
 * 1 = 0*100 +1
 * 
 * 1,1 = 100
...
(function($)
{
	...
	/*
	 * create table with head for anchor for example: <h2 id="#Linux基础">Linux基础</h2> this method can get a headable anchor add by https://github.com/chanble
	 */
	...
	function _add_header_node(opts, header_obj)
	{
		...
		opts._header_offsets.push($(header_obj).offset().top - opts.highlight_offset + 42);
	...
	function bind_scroll_event_and_update_postion(opts)
	{
		if (opts.highlight_on_scroll)
		{
			...
			{
				var headerArr = $("#MainArticleDiv").find(":header");
				for (var i = 0; i < headerArr.length; i++)
				{
					var obj = $(headerArr[i]);
					if ((obj.offset().top - $(document).scrollTop() > 0) && (obj.offset().top - $(document).scrollTop() < $(window).height()))
					{
						$("#tree").find('a').removeClass('curSelectedNode');
						$('a[title="' + obj.text().trim() + '"]').addClass('curSelectedNode');
						break;
					}
				}
			});
	...
	$.fn.ztree_toc.defaults =
	{
		...
		debug : true,
		/*
		 * 使用标题作为anchor create table with head for anchor for example: <h2 id="#Linux基础">Linux基础</h2>
		 */
		...
		ztreeSetting :
		{
			...
			callback :
			{
				...
				onClick : function(event, treeId, treeNode)
				{
					...
					$("html, body").animate(
					{
						scrollTop : ($(treeNode.url.substring(treeNode.url.indexOf("#"))).offset().top - 60) + "px"
					},
					{
						duration : 500,
						easing : "swing"
